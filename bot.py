...
# ✅ Cambiati i ticker nel formato standard richiesto da Twelve Data
...
# ✅ Nuova variabile d'ambiente obbligatoria per Twelve Data
...
# 3. PERSISTENZA STATO (Mantenuta logica originale v4.0)
...
# 6. FETCH DATI CON VALIDAZIONE (Motore Risviluppato per Twelve Data)
...
def fetch_candles(symbol, interval, outputsize=100):
    ...
    Scarica candele OHLCV da Twelve Data.

    Ritorna (closes, candles) oppure (None, None) se ci sono anomalie.
    ...
    try:
        ...
        if "values" not in r:
            ...
            print(f"[WARN] Twelve Data errore per {symbol} ({interval}): {r.get('message', 'No message')}")

            return None, None



        # Twelve Data restituisce i dati dal più recente al più vecchio.

        # Li invertiamo per allinearli alla logica sequenziale degli indicatori matematici.
        ...
        if len(closes) < (outputsize * 0.6): # Tolleranza minima candele valide ricevute

            print(f"[WARN] {symbol} {interval}: Dati insufficienti ({len(closes)} candele)")
        ...
        if price <= 0 or price > 1000:
            ...
            print(f"[WARN] {symbol}: Prezzo fuori scala ({price})")
        ...
        print(f"[ERRORE] Exception su fetch_candles TwelveData {symbol} {interval}: {e}")
...
# 8. CALCOLO H4 REALE (Mappato su API native Twelve Data)

# ---------------------------------------------------------

def get_ema20_h4_reale(symbol):

    """Scarica candele a 4 ore native e calcola l'EMA 20."""
...
def calcola_matrice_asset(symbol):
    ...
    # Dati M15 (outputsize 100 per avere storico su sufficienza indicatori)
    ...
    # EMA 20 H4 Reale da Twelve Data
    ...
    if ema20_h4 is None:
        ...
        print(f"[WARN] {symbol}: fallback EMA H4 approssimata su H1")
    ...
    if any(v is None for v in [ema50_15m, ema50_1h, ema20_h4, bb_upper, bb_lower, atr]):
        ...
        return {"symbol": symbol, "punti": 0, "direzione": "NESSUNO", "price": price, "msg": "Dati Twelve Data in elaborazione..."}
    ...
    return {
        ...
        "msg":       f"Filtri superati ({totale}/11)" if totale >= SOGLIA_APPROVAZIONE else f"Sotto soglia minima ({SOGLIA_APPROVAZIONE})"
...
def genera_report_ispettivo():
    ...
    if segnale_in_attesa["attivo"]:
        ...
        report += "⏳ *Stato*: In attesa di conferma entrata...\n-------------------------\n"
...
# 10. MONITORAGGIO TRADE ATTIVO (Precisione M1 Twelve Data)
